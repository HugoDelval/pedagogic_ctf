...
def check(binary, randomize):
    streamdata_ref, ret_ref = run_cmd(["/usr/bin/dig", "+short", "www.ovh.com"])
    ...
    if streamdata_ref_set != streamdata_bin_set:
        print("Output of `./youcode_compiled www.ovh.com` different from `/usr/bin/dig +short www.ovh.com`")
        print("\n\nYour code outputs : \n" + streamdata_bin + "\n\nAnd `dig` outputs : \n" + streamdata_ref)
        return False

    streamdata_ref, ret_ref = run_cmd(["/usr/bin/dig", "+short", "8.8.8.8"])
    ...
    streamdata_bin_set = clean_output(streamdata_bin)
    if streamdata_ref_set != streamdata_bin_set:
        print("Output of `./youcode_compiled 8.8.8.8` different from `/usr/bin/dig +short 8.8.8.8`")
        print("\n\nYour code outputs : \n" + streamdata_bin + "\n\nAnd `dig` outputs : \n" + streamdata_ref)
        return False
    ...
    if ret != 0 or streamdata != "Error, server param not valid.\n":
        print(
            "Output of `./youcode_compiled \"test with w!erds charac|eres\"` should be :\n\"Error, server param not "
            "valid.\"\n\nBut the output is :\n\"" + streamdata + "\"")
        return False

    streamdata, ret = run_cmd([binary])
    if ret != 0 or streamdata != "Please send me a server address so I can dig it ! (ex: google.com or www.ovh.com)\n":
        print(
